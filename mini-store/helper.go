...
//
// 出现非零 errcode 时返回 *APIError；当响应不是合法 JSON envelope（例如
// 代理返回的 HTML 错误页或二进制响应）时返回带原因的错误，避免"静默
// unmarshal"把损坏响应当成功。
//
...
func (c *Client) doPost(ctx context.Context, path string, body any, out any) error {
	...
	if err != nil {
		return fmt.Errorf("mini-store: %s: marshal request: %w", path, err)
	}
...
func decodeEnvelope(path string, respBody []byte, out any) error {
	...
	if err := json.Unmarshal(respBody, &base); err != nil {
		return fmt.Errorf("mini-store: %s: decode envelope: %w (body snippet: %s)",
	...
	if out != nil {
		if err := json.Unmarshal(respBody, out); err != nil {
			return fmt.Errorf("mini-store: %s: decode result: %w", path, err)
		}
	}
	return nil
}
