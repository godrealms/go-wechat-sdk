...
func (c *Client) TranslateContent(lfrom, lto, content string) (*TranslateContentResult, error) {
	// 构造请求URL
	path := fmt.Sprintf("/cgi-bin/media/voice/translatecontent?lfrom=%s&lto=%s&access_token=%s",
		url.QueryEscape(lfrom), url.QueryEscape(lto), c.GetAccessToken())
...
func (c *Client) AddVoiceToRecoForText(format, voiceID, lang string, media []byte) (*Resp, error) {
	// 构造请求URL
	path := fmt.Sprintf("/cgi-bin/media/voice/addvoicetorecofortext?format=%s&voice_id=%s&lang=%s&access_token=%s",
		url.QueryEscape(format), url.QueryEscape(voiceID), url.QueryEscape(lang), c.GetAccessToken())
...
func (c *Client) QueryRecoResultForText(voiceID, lang string) (*QueryRecoResultForTextResult, error) {
	// 构造请求URL
	path := fmt.Sprintf("/cgi-bin/media/voice/queryrecoresultfortext?voice_id=%s&lang=%s&access_token=%s",
		url.QueryEscape(voiceID), url.QueryEscape(lang), c.GetAccessToken())
