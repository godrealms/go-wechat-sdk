...
// (not a plain fmt.Errorf string) when WeChat returns a non-zero errcode,
// so callers can errors.As() it. Audit Batch 1.
...
func TestDoPost_DecodesSuccessIntoOut(t *testing.T) {
	srv := fakeServer(t, func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"sku_id":"s1","price":1099}`))
	})
	defer srv.Close()
	...
	if out.SKUID != "s1" || out.Price != 1099 {
