...
//
// 出现非零 errcode 时返回 *APIError，调用方可用 errors.As 区分；
// 当响应不是合法 JSON envelope（例如代理返回的 HTML 错误页或二进制响应）时
...
func (c *Client) doPost(ctx context.Context, path string, body any, out any) error {
	...
	if err != nil {
		return fmt.Errorf("channels: %s: marshal request: %w", path, err)
	}
...
func decodeEnvelope(path string, respBody []byte, out any) error {
	...
	if err := json.Unmarshal(respBody, &base); err != nil {
		return fmt.Errorf("channels: %s: decode envelope: %w (body snippet: %s)",
	...
	if out != nil {
		if err := json.Unmarshal(respBody, out); err != nil {
			return fmt.Errorf("channels: %s: decode result: %w", path, err)
		}
	}
	return nil
}
