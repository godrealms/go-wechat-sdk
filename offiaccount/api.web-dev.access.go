...
func (c *Client) GetSnsUserInfo(ctx context.Context, accessToken, openID, lang string) (*SnsUserInfo, error) {
	// 如果语言参数为空，默认使用简体中文
	if lang == "" {
		lang = "zh_CN"
	}
...
// accessToken: 网页授权接口调用凭证
// openID: 用户的唯一标识
func (c *Client) CheckSnsAccessToken(ctx context.Context, accessToken, openID string) (*Resp, error) {
	// 构造请求URL
