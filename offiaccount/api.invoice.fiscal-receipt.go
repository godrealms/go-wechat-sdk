...
func (c *Client) GetTicket(ctx context.Context) (*GetTicketResult, error) {
	...
	if err := c.Https.Get(ctx, "/cgi-bin/ticket/getticket", query, result); err != nil {
		return nil, err
	}
	return result, nil
}
...
func (c *Client) SetPlatformPdf(ctx context.Context, pdfFilePath string) (*SetPdfResult, error) {
	token, err := c.AccessTokenE(ctx)
	if err != nil {
		return nil, err
	}
	...
	// 使用通用的Post方法发送文件
	err = c.Https.Post(ctx, path, pdfFilePath, result)
	return result, err
}
